...
def run(args):
    ...
    myForces = forces.set_up_forces(oa, submode=args.subMode, contactParameterLocation=parametersLocation, memory=args.memory, memory_strength = args.memoryStrength, memory_only=args.memoryOnly)
    # print(forces)
    ...
    if args.includeLigands:
        ...
        additional_cmd = ""
    os.system(f"{sys.executable} mm_analyze_custom_memory.py {args.protein} -t {os.path.join(toPath, 'movie.dcd')} --subMode {args.subMode} -f {args.forces} {analysis_fasta} {additional_cmd} -c {chain} --memory {args.memory} --memoryStrength {args.memoryStrength} --memoryOnly {args.memoryOnly}")
