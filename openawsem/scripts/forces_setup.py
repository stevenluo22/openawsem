...
def set_up_forces(oa, computeQ=False, submode=-1, contactParameterLocation=".", membrane_center=-0*angstrom, memory="frags.mem", memory_strength=1, memory_only=False):
    # apply forces
    if memory_only == True:
        forces = [
            templateTerms.fragment_memory_term(oa, k_fm = memory_strength * 0.04184, frag_file_list_file=memory, UseSavedFragTable=False)
        ]
    else:
        forces = [
